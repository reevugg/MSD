...
def button_click_handler(button_value):
    if not button_locked[button_value]:
        ...
        button_states[button_value].config(state=tk.DISABLED)  # Disable the main button
        return_states[button_value].config(state=tk.NORMAL)  # Enable the return button

# Function to handle return button click and enable the main button
def return_button_click_handler(button_value):
    if button_locked[button_value]:
        custom_return_value = custom_return_data.get(button_value, "")
        if custom_return_value:
            data_list = custom_return_value.split(',')
            for data in data_list:
                send_numeric_data(data)
                time.sleep(2)  # Pause for 2 seconds between data sends
        button_states[button_value].config(state=tk.NORMAL)  # Enable the main button
        return_states[button_value].config(state=tk.DISABLED)  # Disable the return button
        root.update_idletasks()  # Update the GUI immediately to gray out the return button
...
def update_data_table():
    for i in range(1, 6):
        ...
        data_table.item(f"item{i}_main", values=(f"Package {i}", custom_value))
...
for i in range(1, 6):
    main_button = tk.Button(root, text=f"Package {i}", command=lambda i=i: button_click_handler(i))
...
for i in range(1, 6):
    ...
    data_table.insert("", "end", iid=f"item{i}_main", values=(f"Package {i}", custom_value))
