...
# Function to handle return button click and send data once
...
for i in range(1, 6):
    ...
    return_button = tk.Button(root, text=f"Return {i}", command=lambda i=i: return_button_click_handler(i))
...
# Create a data table
data_table = ttk.Treeview(root, columns=("Item", "Custom Data (Main)", "Custom Data (Return)"), show="headings")
data_table.heading("Item", text="Item")
data_table.heading("Custom Data (Main)", text="Custom Data (Main)")
data_table.heading("Custom Data (Return)", text="Custom Data (Return)")
data_table.pack()
